...
export function LightBloom({
  ...
  const blurSize = variant === 'landing' ? 'blur-[190px]' : 'blur-[220px]'
  const d = variant === 'landing' ? 72 : 60
  const box = variant === 'landing' ? 'absolute -inset-[3%] min-h-[106%] min-w-[106%] z-0' : 'absolute inset-0 z-0'

  return (
    <motion.div
      animate={reducedEffects ? undefined : { x: [0, '0.4%', 0], y: [0, '-0.2%', 0] }}
      className={`light-bloom-wrap pointer-events-none overflow-hidden ${box} ${bloomOpacity}`}
      transition={reducedEffects ? { duration: 0 } : { duration: d, ease: 'easeInOut', repeat: Infinity }}
    >
      ...
      <div className="absolute right-[20%] top-[52%] h-[26rem] w-[26rem] rounded-full bg-amber-300/10 blur-[170px]" />
    </motion.div>
  )
}
